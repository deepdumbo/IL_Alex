function [iField,voxel_size,matrix_size,CF,delta_TE,TE,B0_dir,files]=Read_DICOM(DicomFolder)
...
%         end
        ...
        for i = 1:length(filelist)
            filename = fullfile(DicomFolder,filelist(i).name);
%             info = dicominfo(filename);
            info = get_dcm_tags(filename, tags);
                        ...
                        case 'SQ'
                            for j=1:length(attrs(k).Data)
                                info.(t).(['Item_' num2str(j)])=...
                                    get_dcm_tags(attrs(k).Data(j).Data, tags);
                            end
                        otherwise
                            error(['get_dcm_tags cannot deal with VR ' attrs(k).VR...
                                ' for tag ' t]);
